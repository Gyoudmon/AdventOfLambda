...
@handbook-footnote{@idea{程序代码应当要方便人类阅读，其次才是计算机执行。}}。
...
@tamer-repl[(code:comment "对空列表求和得到0")
            (apply + (list))
            (apply + (list 1 2 3))
            (apply + (list 4 5 6))
...
最后，但依然重要的一步，
针对列表设计的确定最大值逻辑：
@desc{将当前精灵所携带的卡路里插入到列表中更小的元素之前，
 并踢掉列表尾部的元素}。

不太好理解的样子。
给自己一点耐心，不妨先拿出纸笔来模拟一下这个过程。
...
是作用于各种数据结构最常见的操作，@tech{列表}则是其中最简单的代表。
因此提供@tech{列表}的语言也会同时为我们提供一大堆@tech{列表}操作函数，
...
于是，@handbook-chunk[<更新卡路里列表>
                   (let-values ([(tops tail) <拆分列表>])
                     (if (null? tail) tops <组装新列表>))]

跟其他 (绝大多数) 语言不同的是，Racket 函数可以有@focus{多个}返回值。
很明显@id{<拆分列表>}会有两个返回值，
分别表示与当前精灵相比的高卡路里列表（@variable{tops}）和低卡路里列表（@variable{tail}），
因此需要与@id{let-values}配合、准备两个变量来分别@term-name{绑定}两个返回值。
当返回的第二个列表为空时，说明当前精灵卡路里没有入围，
此时的@variable{tops}即为下一轮循环的初始值。

@note{Python 可以通过自动序列装包/拆包达到同样的目的，但那仍然只是返回一个值。
 其具体过程是：将多个返回值搜集进一个元组，然后将这个元组作为单个值返回，
 接着再一个一个把这个元组里的值取出来分别赋值给多个变量。}

@handbook-chunk[<拆分列表>
                (splitf-at calories (curry < self:cal))]

@tech{高阶函数}@id{curry}用在这里犯规了，它的输入和输出都是函数。
仅就本题而言，它接受小于操作符（@id{<}）和精灵卡路里，
返回一个@tech{谓词函数}，用以决定列表中的元素将被追加到哪一个返回的列表中。
小于操作符（@id{<}）至少需要两个操作数，我们只能提供第一个。
返回的@tech{谓词函数}的输入参数充当第二个操作数，
由@racket[splitf-at]在拆分列表的过程中依次提供。

@tech{高阶函数}@id{splitf-at}一次返回@focus{两个}列表。
其第二个参数是一个@tech{谓词函数}，
用于依次比较列表中的元素是否比当前卡路里大，
是就追加到第一个返回的列表中；
否则追加到第二个返回的列表中。

@handbook-chunk[<组装新列表>
                (append tops
                        (cons self:cal
                              (drop-right tail 1)))]

碎片@id{<组装新列表>}看上去挺吓人的，
实际上都是@tech{列表}最基本的操作。
其中特别需要说明的是@id{cons}函数，
这是@tech{广义表}的专属操作，
用于将一个元素添加到列表的开头。
详细用法在后续任务中再谈。
