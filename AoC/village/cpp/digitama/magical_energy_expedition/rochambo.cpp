...
void WarGrey::AoC::RochamboPlane::reflow(float width, float height) {
    this->move_to(this->titlet, { this->agent, MatterPort::RB }, MatterPort::LB);
    this->move_to(this->population, { this->agent, MatterPort::LB }, MatterPort::LT);
    this->move_to(this->guessed_score, { this->population, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    this->move_to(this->designed_score, { this->guessed_score, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    this->move_to(this->random_score, { this->designed_score, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    
    this->backdrop->resize(width, 0.0F);
    this->move_to(this->backdrop, { 0.0, height }, MatterPort::LB);
    ...
    this->move_to(this->snack, { this->race_x0 + this->race_distance, this->race_y0 }, MatterPort::LC);
    ...
    /* reflow scores */ {
        ...
        for (int idx = 0; idx < sizeof(this->play_icons) / sizeof(Sprite*); idx ++) {
            if (prev == nullptr) {
                this->move_to(this->play_scores[idx], { width, 0.0F }, MatterPort::RT, { -8.0F, 8.0F });
            } else {
                this->move_to(this->play_scores[idx], { prev, MatterPort::LC }, MatterPort::RC, { -xdist, 0.0F });
            }
            this->move_to(this->play_icons[idx], { this->play_scores[idx], MatterPort::LC }, MatterPort::RC);
            prev = this->play_icons[idx];
        }

        for (int idx = 0; idx < sizeof(this->outcome_icons) / sizeof(Sprite*); idx ++) {
            this->move_to(this->outcome_icons[idx], { this->play_icons[idx], MatterPort::CB }, MatterPort::CT);
            this->move_to(this->outcome_scores[idx], { this->play_scores[idx], 0.5F, this->outcome_icons[idx], 0.5F }, MatterPort::CC);
        }
    }
}

void WarGrey::AoC::RochamboPlane::on_mission_start(float width, float height) {
    this->move_to(this->tux, { this->tent, { tux_fx_in_tent, tux_fy_in_tent }}, MatterPort::CC);

    this->elves[0]->scale_to(competitor_scale);
    this->elves[1]->scale_to(competitor_scale);

    this->move_to(this->elves[0], { this->round_x0, this->round_y0 }, MatterPort::RC);
    this->move_to(this->elves[1], { this->round_x0, this->round_y0 }, MatterPort::LC);
    ...
    this->move_to(this->l_play, { this->elves[0], MatterPort::CT }, MatterPort::CB);
    this->move_to(this->r_play, { this->elves[1], MatterPort::CT }, MatterPort::CB);
        ...
        if ((m == this->guessed_score) || (m == this->designed_score) || (m == this->random_score) || (m == this->tux)) {
            ...
            if (this->round_self == nullptr) {
                if (m == this->elves[0]) {
                    ...
                    this->move_to(this->outcome_desc, { this->elves[0], MatterPort::LT }, MatterPort::RT);
                    this->round_score(this->r_play, this->l_play);
                } else if (m == this->elves[1]) {
                    this->round_self = this->r_play;
                    this->move_to(this->outcome_desc, { this->elves[1], MatterPort::RT }, MatterPort::LT);
...
void WarGrey::AoC::RochamboPlane::on_task_start(RPSStatus status) {
    ...
    this->glide_to(moving_duration_when_race_start, this->tux, { this->race_x0, this->race_y0 }, MatterPort::CC);
    this->glide_to(moving_duration_when_race_start, this->elves[0], { this->race_x0, this->race_y0 }, MatterPort::CB, { 0.0F, -yoff });
    this->glide_to(moving_duration_when_race_start, this->elves[1], { this->race_x0, this->race_y0 }, MatterPort::CT, { 0.0F, +yoff });

    this->move_to(this->l_play, { this->snack, MatterPort::LT }, MatterPort::LB);
    this->move_to(this->r_play, { this->snack, MatterPort::RT }, MatterPort::RB);
}
...
void WarGrey::AoC::RochamboPlane::on_task_done() {
    ...
    this->glide_to(moving_duration_when_race_end, this->tux, { this->tent, { tux_fx_in_tent, tux_fy_in_tent }}, MatterPort::CC);
    this->glide_to(moving_duration_when_race_end, this->elves[0], { this->round_x0, this->round_y0 }, MatterPort::RC);
    this->glide_to(moving_duration_when_race_end, this->elves[1], { this->round_x0, this->round_y0 }, MatterPort::LC);
...
void WarGrey::AoC::RochamboPlane::on_motion_step(IMatter* m, float x, float y, double xspd, double yspd, double percentage) {
    if (yspd <= 0.0F) {
        if (this->elves[0] == m) {
            this->move_to(this->l_play, { m, MatterPort::CT }, MatterPort::CB);
        } else if (this->elves[1] == m) {
            this->move_to(this->r_play, { m, MatterPort::CT }, MatterPort::CB);
        }
    }
}
...
int WarGrey::AoC::RochamboPlane::round_score(Sprite* opponent, Sprite* self) {
    ...
    switch (outcome) {
        case RPSOutcome::Lose: {
            ...
            this->outcome_desc->set_text(MatterPort::CC, outcome_fmt, shpscore, outscore);
        }; break;
        case RPSOutcome::Win: {
            if (this->round_self == this->l_play) {
                this->elves[0]->play("rshoot");
