...
void WarGrey::AoC::RucksackReorganizationPlane::reflow(float width, float height) {
    ...
    this->move_to(this->misplaced_sum, { this->population, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    this->move_to(this->badge_sum, { this->misplaced_sum, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    
    this->backdrop->resize(width, 0.0F);
    this->move_to(this->backdrop, { 0.0F, height * 0.70F }, MatterPort::LB);
    
    this->move_to(this->backpack, { this->backdrop, MatterPort::LT }, MatterPort::LB, { 0.0F, -2.0F });
    this->move_to(this->info_board, { this->backdrop, MatterPort::RT }, MatterPort::RB, { 0.0F, -2.0F });
    this->move_to(this->hashtable, { this->misplaced_sum, MatterPort::RT }, MatterPort::LT, { float(generic_font_size(FontSize::xx_large)), 0.0F});
...
void WarGrey::AoC::RucksackReorganizationPlane::move_rucksack_to_grid(Rucksack* rs) {
    this->move_to_grid(rs, int(rs->id), MatterPort::CB);
}
...
void WarGrey::AoC::RucksackReorganizationPlane::on_task_done() {
    this->status = RRStatus::TaskDone;
    this->info_board->set_text(MatterPort::RB, "");
    this->backpack->clear();
    this->hashtable->clear();
    this->agent->stop(1);
}
...
void WarGrey::AoC::RucksackReorganizationPlane::display_items(Rucksack* self) {
    ...
    this->info_board->set_text(items, MatterPort::RB);
...
void WarGrey::AoC::Backpack::construct(dc_t* dc) {
    GridAtlas::construct(dc);
    this->create_map_grid(2, 26, rucksack_item_size, rucksack_item_size);
}

void WarGrey::AoC::Backpack::draw(dc_t* dc, float x, float y, float Width, float Height) {
    GridAtlas::draw(dc, x, y, Width, Height);

    for (auto s_item : this->shared_indices) {
        Box box = this->get_map_tile_region(s_item);

        dc->draw_rect(box.ltdot.x + x, box.ltdot.y + y, box.width(), box.height(), ROYALBLUE);
    }
}
...
void WarGrey::AoC::PackHash::construct(dc_t* dc) {
    GridAtlas::construct(dc);
    this->create_map_grid(2, 16, rucksack_item_size * hash_scale, rucksack_item_size * hash_scale);
}

void WarGrey::AoC::PackHash::draw(dc_t* dc, float x, float y, float Width, float Height) {
    GridAtlas::draw(dc, x, y, Width, Height);
    ...
    for (auto it = this->dict.begin(); it != this->dict.end(); it ++) {    
        dc->draw_blended_text(std::to_string(it->second), GameFont::monospace(), cx + xoff, cy, GRAY);
        cx += dx;
    }
}
