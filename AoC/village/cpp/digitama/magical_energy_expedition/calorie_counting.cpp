...
void WarGrey::AoC::CalorieCountingPlane::load(float width, float height) {
    ...
    if (this->elves.size() > 0) {
        ...
        for (auto elf : this->elves) {
            this->insert(elf, float(random_uniform(x0, xn)), float(random_uniform(y0, yn)), MatterPort::CC);
...
void WarGrey::AoC::CalorieCountingPlane::reflow(float width, float height) {
    ...
    this->move_to(this->snack, { width, 0.0F }, MatterPort::RT);
    this->move_to(this->info_board, { this->snack, MatterPort::LC }, MatterPort::RC);
    this->move_to(this->population, { this->agent, MatterPort::LB }, MatterPort::LT);
    this->move_to(this->top1_total, { this->population, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    this->move_to(this->topn_total, { this->top1_total, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    this->move_to(this->sorted_total, { this->topn_total, MatterPort::LB }, MatterPort::LT, { 0.0F, 1.0F });
    
    this->backdrop->resize(width, height);
    this->move_to(this->backdrop, { 0.0F, height }, MatterPort::LB);
    ...
    if (this->elves.size() > 0) {
        ...
        /* reflow top elves labels */ {
            ...
            for (int idx = 0; idx < this->dims.size(); idx ++) {
                ...
                this->move_to(this->dims[idx], { (c + 0.4F) * cwidth + xoff, (r + 1.0F) * cheight + yoff }, MatterPort::CB);
            }
        }
    }
}
    ...
    switch (this->status) {
        case CCStatus::CountOff: {
            ...
            if (this->current_elf_idx < this->elves.size()) {
                ...
                this->info_board->set_text(std::to_string(this->elves[this->current_elf_idx]->calorie_total()), MatterPort::RC);
        ...
        case CCStatus::FindMaximumCalorie: {
            if (this->current_elf_idx < this->elves.size()) {
                ...
                this->info_board->set_text(std::to_string(self_cal), MatterPort::RC);
                ...
                if (self_cal > this->top_calorie) {
                    ...
                    this->glide_to(moving_duration_when_exciting, this->elves[this->current_elf_idx],
                        { this->top1_total, MatterPort::RB }, MatterPort::LB);
    ...
    if (yes_or_no) {
        if (m == this->population) {
            ...
            if (maybe_elf != nullptr) {
                this->info_board->set_text(std::to_string(maybe_elf->calorie_total()), MatterPort::RC);
            }
        }
...
void WarGrey::AoC::CalorieCountingPlane::move_elf_to_grid(Elfmon* elf) {
    elf->motion_stop();
    this->glide_to_grid(moving_duration_when_calming_down, elf, elf->id, MatterPort::CB);
}
...
void WarGrey::AoC::CalorieCountingPlane::reflow_top_elves() {
    for (int idx = 0; idx < this->top_elf_indices.size(); idx ++) {
        ...
        if (top_elf_idx >= 0) {
            ...
            this->glide_to(moving_duration_when_exciting, this->elves[top_elf_idx],
                { this->dims[idx], MatterPort::CT }, MatterPort::CB);
        } else {
            break;
        }
    }
...
void WarGrey::AoC::CalorieCountingPlane::random_walk(int start_idx) {
    for (int idx = start_idx; idx < this->elves.size(); idx ++) {
        this->glide(moving_duration_when_random_walking, this->elves[idx],
            { float(random_uniform(-micro_pace, micro_pace)),
              float(random_uniform(-micro_pace, micro_pace)) });
    }
}
...
void WarGrey::AoC::CalorieCountingPlane::on_task_done() {
    ...
    this->info_board->set_text(MatterPort::RC, "");
    this->agent->stop(1);
}
...
void WarGrey::AoC::CalorieCountingPlane::on_motion_complete(IMatter* m, float x, float y, double xspd, double yspd) {
    ...
    if (elf != nullptr) {
        Box box = elf->get_bounding_box();

        if (box.width() > elf_size) {
            elf->play("dspellcast");
        } else {
            elf->play("lwalk");
        }
